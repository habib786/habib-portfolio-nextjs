...
import ExperienceTimeline, {
  TimelineEvent,
  timelineData as fallbackTimelineData,
...
export default function ExperienceEducationSection() {
  ...
  const [timelineData, setTimelineData] = useState<TimelineEvent[] | undefined>(
    undefined,
  );
      ...
      try {
        ...
        if (eduData.length > 0) {
          mappedEdu = eduData.map((e: any) => ({
            ...
            date: e.year_range,
            icon: iconMap[e.icon] || GraduationCap,
          }));
        } else {
          // Hardcoded fallback if nothing in DB
          mappedEdu = fallbackTimelineData.filter(
            (e) => e.type === "education",
          );
        }
        ...
        if (expData.length > 0) {
          mappedExp = expData.map((e: any) => ({
            ...
            icon: iconMap[e.icon] || Briefcase,
          }));
        } else {
          // Hardcoded fallback if nothing in DB
          mappedExp = fallbackTimelineData.filter(
            (e) => e.type === "experience",
          );
        }
