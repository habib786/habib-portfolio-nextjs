...
export default function ClientsMarquee({ clients }: ClientsMarqueeProps) {
  ...
  const smoothXTranslate = useSpring(xTranslate, { stiffness: 100, damping: 30 });
  ...
  const itemX = useSpring(useTransform(mouseX, [-0.5, 0.5], [-15, 15]), { stiffness: 100, damping: 30 });
  const itemY = useSpring(useTransform(mouseY, [-0.5, 0.5], [-10, 10]), { stiffness: 100, damping: 30 });
            ...
            >
              {client.logo_type === 'text' && (
                <>
                  ...
                  >
                    <span className="font-black text-sm md:text-base">{client.icon?.charAt(0) || client.name.charAt(0)}</span>
                  </motion.div>
