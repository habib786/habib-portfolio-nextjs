...
const defaultProjects = [
  {
    ...
    description: 'An intelligent resume builder that uses AI to optimize content and design for ATS systems.',
  ...
  },
  {
    ...
    technologies: ['React Native', 'Express.js', 'MongoDB', 'Firebase', 'Chart.js'],
  ...
  },
  {
    ...
    description: 'Property management platform with virtual tours, tenant management, and financial tracking.',
          ...
          <Grid size={{ xs: 12, md: 6 }} key={project.id}>
            <Card 
              ...
              sx={{ 
                ...
                border: '1px solid',
                borderColor: hoveredProject === project.id ? 'rgba(16,106,90,0.4)' : 'divider',
                boxShadow: hoveredProject === project.id ? '0 30px 60px rgba(16,106,90,0.15), inset 0 1px 0 rgba(255,255,255,0.2)' : '0 10px 30px rgba(0,0,0,0.05), inset 0 1px 0 rgba(255,255,255,0.1)',
                transition: 'all 0.5s cubic-bezier(0.25, 1, 0.5, 1)',
                transform: hoveredProject === project.id ? 'translateY(-10px) scale(1.02)' : 'none',
                '&::before': {
                  content: '""',
                  position: 'absolute',
                  top: '-50%',
                  ...
                  width: '200%',
                  height: '200%',
                  background: 'radial-gradient(circle, rgba(16, 106, 90, 0.08) 0%, transparent 50%)',
            ...
            >
              ...
              <Box sx={{ 
                ...
                <Typography variant="h2" sx={{ 
                  fontWeight: 900, 
                  color: hoveredProject === project.id ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.05)' 
                }}>
              ...
              <CardContent sx={{ flexGrow: 1, p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                  <Typography variant="h5" component="h3" sx={{ 
                    fontWeight: 700, 
                    color: hoveredProject === project.id ? 'primary.main' : 'text.primary',
                  ...
                  <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    {project.category && (
                      <Chip label={project.category} size="small" color="primary" sx={{ fontSize: 10, bgcolor: 'primary.50' }} />
                    )}
                  </Stack>
                </Box>
                ...
                <Stack direction="row" sx={{ gap: 1, flexWrap: 'wrap' }}>
                  {(project.technologies || []).slice(0, 4).map((tech: string) => (
                    <Chip key={tech} label={tech} size="small" variant="outlined" sx={{ fontSize: 10, height: 20 }} />
                  ))}
                  {(project.technologies || []).length > 4 && (
                    <Chip label={`+${(project.technologies || []).length - 4}`} size="small" variant="outlined" sx={{ fontSize: 10, height: 20 }} />
                  )}
                </Stack>
              </CardContent>
              ...
              <CardActions sx={{ p: 3, pt: 0, justifyContent: 'space-between' }}>
                <Button variant="ghost" size="sm" component={Link} href={`/projects/${project.slug || project.id}`}>
                  View Details
                </Button>
                <Stack direction="row" spacing={1}>
                  {project.github_url && (
