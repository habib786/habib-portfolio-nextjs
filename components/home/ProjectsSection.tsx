...
export default function ProjectsSection() {
  ...
  return (
    <Box component="section" ref={containerRef} sx={{ py: 12, bgcolor: 'background.default', position: 'relative', overflow: 'hidden' }}>
      ...
      <motion.div 
        style={{ x: xBg }}
        className="absolute top-1/2 left-0 text-[10rem] md:text-[15rem] font-bold text-gray-50 dark:text-gray-900/10 pointer-events-none whitespace-nowrap z-0 select-none"
      >
        PORTFOLIO WORKS
      </motion.div>
    ...
    >
      <Card sx={{ 
        ...
        '&:hover': {
          ...
          boxShadow: '0 30px 60px rgba(16,106,90,0.2), inset 0 1px 0 rgba(255,255,255,0.2)',
          borderColor: 'rgba(16,106,90,0.4)'
        },
        '&:hover .project-overlay': { opacity: 1, backdropFilter: 'blur(10px)' },
        '&:hover img': { scale: '1.1' },
        ...
        <Box sx={{ aspectRatio: '4/3', width: '100%', position: 'relative', overflow: 'hidden' }}>
          ...
          <Box className="project-overlay" sx={{ 
            position: 'absolute', 
            inset: 0, 
            background: 'linear-gradient(to top, rgba(9, 9, 11, 0.95) 0%, rgba(16, 106, 90, 0.7) 40%, transparent 100%)', 
            ...
            />
            <Typography variant="h4" sx={{ color: 'white', fontWeight: 900, mb: 3, textTransform: 'uppercase', letterSpacing: 1 }}>
              {project.title}
            </Typography>
            <Button 
              variant="default" 
